  ...
  try{
    ...
    const sql = `
      SELECT
        (SELECT SUM(stok) 
          FROM paket_bansos
          WHERE id_penyedia = ? AND is_deleted = 0) AS jumlah_stok,
  
        (SELECT COUNT(DISTINCT tb.id_transaksi)
          FROM transaksi_bansos tb
          JOIN paket_bansos pb ON tb.id_paket = pb.id_paket
          WHERE pb.id_penyedia = ? AND is_deleted is 0) AS telah_terdistribusi;
      `;
...
exports.deletePaket = async (req, res) => {
  ...
  try {
    const [result] = await db.promise().query(
      'UPDATE paket_bansos SET is_deleted = 1 WHERE id_paket = ?',
      [id_paket]
    );
...
exports.editPaket = async (req, res) => {
  ...
  try {
    const [result] = await db.promise().query(
      'UPDATE paket_bansos SET nama_paket = ?, stok = ?, deskripsi = ? WHERE id_paket = ? AND is_deleted = 0',
      [nama_paket, stok, deskripsi, id_paket]
    );
    if (result.affectedRows === 0) {
